<template>
  ...
  <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full animate__animated animate__fadeIn animate__slow animate__delay-2s">
    <div class="row">
      <div class="grid relative left-1/2 transform -translate-x-1/2 grid-cols-2 gap-4 mb-4 p-4 rounded-2xl w-11/12" style="background-color: rgba(0, 0, 0, 0.25);">
        <!-- PREGUNTA -->
        <button class="rounded-full border-black border-4 bg-white drop-shadow-md col-span-2 text-xl antialiased font-bold min-h-[3rem]">
          PREGUNTA
        </button>
        
        <!-- RESPUESTAS -->
        <button v-on:click="this.selectAnswer('A')" class="rounded-tl-full rounded-bl-full rounded-br-full border-black border-4 p-2 bg-white drop-shadow-md antialiased font-bold min-h-[3rem]">
          <span class="absolute -bottom-50 -left-4 z-20 bg-white px-2 rounded-full border-black border-4 text-lg">A</span>
          RESPUESTA 1
        </button>
        <button v-on:click="this.selectAnswer('B')" class="rounded-tl-full rounded-bl-full rounded-br-full border-black border-4 p-2 bg-white drop-shadow-md antialiased font-bold min-h-[3rem]">
          <span class="absolute -bottom-50 -left-4 z-20 bg-white px-2 rounded-full border-black border-4 text-lg">B</span>
          RESPUESTA 2
        </button>
        <button v-on:click="this.selectAnswer('C')" class="rounded-tl-full rounded-bl-full rounded-br-full border-black border-4 p-2 bg-white drop-shadow-md antialiased font-bold min-h-[3rem]">
          <span class="absolute -bottom-50 -left-4 z-20 bg-white px-2 rounded-full border-black border-4 text-lg">C</span>
          RESPUESTA 3
        </button>
        <button v-on:click="this.selectAnswer('D')" class="rounded-tl-full rounded-bl-full rounded-br-full border-black border-4 p-2 bg-white drop-shadow-md antialiased font-bold min-h-[3rem]">
    ...
    <div class="row">
      ...
      <div class="grid relative left-1/2 transform -translate-x-1/2 grid-cols-2 gap-4 mb-4 p-4 rounded-2xl w-11/12" style="background-color: rgba(0, 0, 0, 0.25);">
        ...
        <!-- COMODINES -->
        <button class="flex justify-center items-center rounded-lg border-black border-4 p-2 bg-white drop-shadow-md antialiased font-bold min-h-[4rem]">
  ...
  <div class="absolute bottom-0 w-full animate__animated animate__fadeIn animate__slow animate__delay-2s">
    <div class="row">
  ...
  export default {
    ...
    methods: {
      selectAnswer(respuestaSeleccionada) {
        this.ws.send('Respuesta seleccionada -> '+respuestaSeleccionada)
      }
    }
  }
</script>